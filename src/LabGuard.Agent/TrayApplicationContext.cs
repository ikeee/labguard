...
namespace LabGuard.Agent
{
    ...
    internal sealed class TrayApplicationContext : ApplicationContext, ILockScreen, IAlertScreen
    {
        ...
        public TrayApplicationContext(GuardConfig config, bool dryRun)
        {
            ...
            {
                _paused = true;
                Balloon("机房管理助手", "当前是安全模式：按设置不加载管控。你可以在这种模式下修复或彻底卸载。");
            }
        ...
        private void StartEngine()
        {
            try
            {
                ...
                if (Core.Interop.SafeModeDetector.ShouldSkipEnforcement(_config.SafeMode.RunInSafeMode))
                {
                    _paused = true;
                    UpdateTrayText();
                    Balloon("机房管理助手", "当前是安全模式：不启用监控（如需在安全模式也管控，请在设置里打开\"安全模式也运行\"）。");
                    return;
                }
                ...
                _paused = false;
                UpdateTrayText();
                Balloon("机房管理助手", "监控已启动" + (_dryRun ? "（干跑模式：只记录，不修改系统）" : ""));
            }
            catch (Exception ex)
            {
                Log.Error("启动监控失败", ex);
                Balloon("机房管理助手", "启动监控失败，详见日志");
            }
        }
        ...
        private void PauseEngine()
        {
            ...
            _paused = true;
            UpdateTrayText();
            Balloon("机房管理助手", "已暂停监控：现在可以使用U盘、下载文件、运行被拦截的软件。" +
        ...
        private bool Verify(string action)
        {
            if (string.IsNullOrEmpty(_config.PasswordHash))
            {
                Balloon("机房管理助手", "尚未设置密码，请先运行设置程序。");
                return false;
            }
            using (var dlg = new PasswordDialog("机房管理助手 - " + action, action + "密码：", _config.PasswordHash))
            {
        ...
        private void OpenSettings()
        {
            ...
            if (!File.Exists(exe)) { Balloon("机房管理助手", "未找到设置程序：" + exe); return; }
        ...
        private void ShowStatus()
        {
            ...
            MessageBox.Show(body, "学生机房管理助手（复刻）", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void ShowAbout()
        {
            MessageBox.Show(
                ...
                "基于对原版《学生机房管理助手 v13.03》的原理分析实现，功能等价、源码开放。" + Environment.NewLine +
        ...
        {
            string state = _paused ? "已暂停" : (_engine.IsRunning ? "监控中" : "未启动");
            _tray.Text = ("机房管理助手（复刻） - " + state).Substring(0, Math.Min(60, 8 + state.Length));
        }
        ...
        private void PauseBecauseTeacherUnlock()
        {
            ...
            _paused = true;
            UpdateTrayText();
            Balloon("机房管理助手", "已按老师密码解除断网遮罩，监控已暂停（设置里保存或重启小助手可恢复）。");
        }
